...
const getItem = async () => {
  ...
  check_list.value.id = response.data.check_list.id
  check_list.value.name = response.data.check_list.name
  response.data.check_list.inputs.forEach((el)=>{
  ...
  response.data.check_list.check_list_tables.forEach((table)=>{
    tables.value.push(
      {
        id:table.id,
        name:table.name,
