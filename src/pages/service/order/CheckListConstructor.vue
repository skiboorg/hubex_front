<template>
  <div class="rounded-box q-mb-lg">
    <div class="page-search">
      <p class="no-margin title text-bold">Конструктор</p>
    </div>
  </div>
  ...
  <div class="rounded-box q-mb-lg" v-for="(table,table_index) in tables" :key="table_index">
    ...
    <q-form @submit.prevent="formTableSubmit">
      ...
      <div class="row q-col-gutter-sm no-wrap">
        <div class="col-2 relative-position" v-for="(input,input_index) in table.inputs" :key="input_index">

          <q-select dense outlined label="Тип ввода" :options="check_list_table_inputs"

                    @update:model-value = 'table.rows = []'
                    v-model="input.input_type" option-label="name" lazy-rules
      ...
      <q-btn v-if="table.inputs.length>0 && table.inputs.filter(x=>x.input_type === null).length===0" label="Добавить ряд" class="q-mb-lg" no-caps @click='addRow(table_index)'/>

      <div v-for="(row,row_index) in table.rows" :key="row_index">
  ...
  <div class="row q-col-gutter-md bg-grey-3 q-mb-lg items-center"
    ...
    <div class="col-1" v-if="check_list.inputs[row_index].input_type?.is_multiple_boolean || check_list.inputs[row_index].input_type?.is_multiple_boolean_with_input">
      <q-btn icon="add" dense flat text-color="positive" @click="addInputInRow(row_index)"/>
    </div>
    ...
    <div class="col-1" >
      <q-btn dense flat text-color="negative" icon="delete" @click="deleteRow(row_index)"/>
    </div>
  </div>
      <q-btn label="Сохранить чеклист" v-if="check_list.inputs.length>0" type="submit" no-caps />
    </q-form>
...
const formSubmit = async () => {
  const resp = await api.post('/api/data/order_create_checklist',{check_list:check_list.value,tables:tables.value})
  useNotify('positive','Чек лист сохранен')
  router.back()
}
...
const addRow = (index) => {
  ...
  table.inputs.forEach((el)=>{
    console.log(el)
    row.push({input:el.input_type,value:el.input_type.is_boolean ? false :null})
  })
