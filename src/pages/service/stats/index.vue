...
const userSelected = async () => {
  ...
  if (selected_date.value){
    const response = await api(`/api/user/find_worker_time_for_stats?user_id=${user.value.id}&date_gte=${selected_date.value.from}&date_lte=${selected_date.value.to}`)
    ...
    <div class="container">
      <div class="rounded-box q-mb-lg">
        <div class="page-search">
          <p class="no-margin title text-bold">Статистика</p>
          <q-space/>
      ...
      <div class="row q-col-gutter-md">
        ...
        <div class="col-12 col-md-8">
          <p class="text-weight-bold" v-if="orders.result?.length>0">Всего : {{orders.result.length}}</p>
